...
- Frontend posts `{contract}`; backend verifies the contract and on-chain priest in production, resolves the priest inbox from the network, creates the group, adds the priest, and stores the `groupId`.
...
```mermaid
flowchart LR
    M[Member] -->|createProposalSetPaused / UpdateConfig / Withdraw / Disband| C[Contract]
...
- Only pause/unpause, fee repricing, and transfers of assets held by the contract are permitted. Token changes remain disabled, and member pool funds cannot be moved.
...
- POST `/join` – verify purchase and invite member
  ...
  - Body: `{ contractAddress, memberAddress, signature, chainId, inboxId? }` (`inboxId` is ignored in normal environments; local/test fallback may use it deterministically when network resolution is unavailable.)
...
- Proposal actions are restricted to an allowlist enforced at creation and execution:
  - `setPausedDAO(bool)`
  - `updateConfigDAO(address,uint256)` — entry fee only
  - `withdrawTreasuryDAO(address,address,uint256,string)`
  - `withdrawAllTreasuryDAO(address,address,string)`
  - `disbandTreasuryDAO()`
