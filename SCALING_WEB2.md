...
## Reality Check
- Contract layer already handles scale, but backend keeps every templ binding in memory (`backend/src/server.js`). At 1M templs this spawns one long-lived `ethers.Contract` listener per templ and stores the full record set in a single Node process, overwhelming memory and the upstream RPC provider.
...
- **Move persistence to a database** (Postgres or similar). Store templ metadata, bindings, proposal state, and scheduling fields. Query per shard instead of keeping the entire set in RAM.
