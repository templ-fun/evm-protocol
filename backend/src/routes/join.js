...
export function joinRouter({ xmtp, groups, hasPurchased, lastJoin }) {
  const router = Router();
  router.post(
    '/join',
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (
          memberInboxId &&
          typeof memberInboxId === 'string' &&
          memberInboxId.length > 0
        ) {
        ...
        if (!inboxId) {
          return res
            .status(503)
            .json({
              error: 'Member identity not registered yet; retry shortly',
            });
        }
        ...
        try {
          if (typeof record.group.addMembers === 'function') {
            ...
            logger.info(
              { member: memberAddress.toLowerCase() },
              'addMembersByIdentifiers([identifier]) succeeded'
            );
          } else {
        ...
        try {
          ...
          try {
            ...
            logger.info(
              { beforeAgg, afterAgg },
              'XMTP API stats around member add'
            );
        ...
        try {
          ...
          logger.info(
            { members },
            'Group members snapshot after member add'
          );
        } catch (e) {
          void e;
        }
