...
import { waitForInboxReady } from '../xmtp/waitForInboxReady.js';
import { logger } from '../logger.js';

export function templsRouter({ xmtp, groups, persist, connectContract }) {
  const router = Router();
  router.post(
    '/templs',
      ...
      try {
        ...
        try {
          ...
          if (err.message && err.message.includes('succeeded')) {
            logger.info(
              { message: err.message },
              'XMTP sync message during group creation - attempting deterministic resolve'
            );
            try {
              await syncXMTP(xmtp);
            } catch (err) {
            ...
            let candidate =
              byDiff.find((c) => c.name === expectedName) ||
              afterList.find((c) => c.name === expectedName);
            if (!candidate) {
              // Fall back to the newest item among the diffs, then overall list
              candidate =
                byDiff[byDiff.length - 1] || afterList[afterList.length - 1];
            }
            group = candidate;
            if (!group) {
              // As a last resort, retry identity-based group creation once.
              group = await xmtp.conversations.newGroupWithIdentifiers([
                priestIdentifierObj,
              ]);
            }
          } else {
        ...
        try {
          ...
          if (
            priestInboxId &&
            typeof priestInboxId === 'string' &&
            priestInboxId.length > 0
          ) {
            priestInbox = priestInboxId.replace(/^0x/i, '');
          } else {
            try {
              if (typeof xmtp.findInboxIdByIdentifier === 'function') {
                priestInbox = await xmtp.findInboxIdByIdentifier(
                  priestIdentifierObj
                );
              }
            } catch (e) {
          ...
          if (priestInbox && typeof group.addMembers === 'function') {
            ...
            logger.info(
              { priestInboxId: priestInbox, ready },
              'Priest inbox readiness before add'
            );
            const beforeAgg =
              xmtp?.debugInformation?.apiAggregateStatistics?.();
            try {
              await group.addMembers([priestInbox]);
              logger.info(
                { priestInboxId: priestInbox },
                'Added priest by inboxId'
              );
            } catch (addErr) {
              const msg = String(addErr?.message || '');
              // Ignore benign cases like already a member or SDK reporting success as an error
              if (!msg.includes('already') && !msg.includes('succeeded'))
                throw addErr;
            }
            try {
              await syncXMTP(xmtp);
            } catch (e) {
              logger.warn({ e }, 'Server sync after priest add failed');
            }
            try {
              const afterAgg =
                xmtp?.debugInformation?.apiAggregateStatistics?.();
              logger.info(
                { beforeAgg, afterAgg },
                'XMTP API stats around priest add'
              );
            } catch (e) {
              void e;
            }
        ...
        try {
          await group.send(
            JSON.stringify({ type: 'templ-created', contract: contractAddress })
          );
        } catch (err) {
        ...
        if (typeof group.updateName === 'function') {
          try {
            ...
            logger.info(
              { message: err.message },
              'XMTP sync message during name update - ignoring'
            );
          }
        }
        ...
        if (typeof group.updateDescription === 'function') {
          try {
            ...
            logger.info(
              { message: err.message },
              'XMTP sync message during description update - ignoring'
            );
          }
        }
        ...
        try {
          ...
          logger.info(
            { message: err.message },
            'XMTP sync message after creation - ignoring'
          );
        }
        ...
        logger.error(
          { err, priestAddress, contractAddress },
