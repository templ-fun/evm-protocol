...
- Typed proposals only (no arbitrary calls). The allowed actions are:
  ...
  - `disbandTreasuryDAO(address token)` - move the full available balance of `token` into a member-distributable pool. When `token == accessToken`, funds roll into the member pool (`memberPoolBalance`) and update the scaled per-member accumulator (`cumulativeMemberRewards`). Dust that cannot be expressed as an integer per member remains in `memberRewardRemainder` until future deposits make a clean split possible. Each member also carries a personal `memberRewardDust` balance so fractional entitlements survive claims and combine with future distributions. When targeting any other ERC-20 or native ETH (`address(0)`), the amount is recorded in an external rewards pool so members can later claim their share with `claimExternalToken`.
  External pools follow the same scaled accounting: `cumulativeRewards` tracks the per-member sum in `1e18` precision, and `rewardRemainder` stores the unallocated dust without requiring joins to iterate over every token. Members keep per-token dust in `memberExternalRewardDust` so fractional leftovers are claimable after future top-ups without governance manually flushing each token.
