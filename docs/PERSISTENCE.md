...
| --- | --- | --- |
| `templ_bindings` | `contract TEXT PRIMARY KEY`, `telegramChatId TEXT`, `priest TEXT`, `xmtpGroupId TEXT`, `bindingCode TEXT` | Durable mapping between templ contracts, chat channels, and the last known priest. `xmtpGroupId` keeps the group conversation id so the backend can hydrate it on restart. `telegramChatId` remains `NULL` until a binding completes; `bindingCode` stores pending binding snippets and survives restarts. Multiple templs may point at the same chat id when communities want a shared announcement channel. |
