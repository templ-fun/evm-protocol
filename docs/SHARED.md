...
- Exposes legacy string builders (`buildDelegateMessage`, `buildMuteMessage`) for compatibility with older mocks; coverage lives in `shared/signing.test.js`.
...
## Tests
- `shared/signing.test.js` locks down the legacy string helpers and doubles as a smoke test.
