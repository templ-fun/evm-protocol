...
export async function deployTempl({
  ...
  if (!group) {
    logger.error('Could not find group after creation; will rely on join step');
    return { contractAddress, group: null, groupId };
  }
...
export async function purchaseAndJoin({
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => '');
    logger.error('purchaseAndJoin: /join failed', { status: res.status, statusText: res.statusText, body });
