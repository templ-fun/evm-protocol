...
## deploy.js
- Deploys `TemplFactory` (when `FACTORY_ADDRESS` is unset) and creates a new templ via `createTemplWithConfig`. The script deploys a new factory by default; only set `FACTORY_ADDRESS` when you deliberately want to reuse an existing factory for recovery workflows.
