...
import { waitFor, delay } from './wait.js';
    ...
    check: async () => {
      ...
      if (!conv) {
        try {
          ...
          logger.debug(
            `Sync attempt: Found ${conversations.length} conversations; firstIds=`,
      ...
      if (conv) {
        logger.debug('Found group:', conv.id, 'consent state:', conv.consentState);
